...
;( function( window ) {
	...
	ElastiStack.prototype._moveAway = function( instance, voteFlag ) {
        ...
        if(typeof angular != "undefined" && voteFlag != "undefined" && $(instance.element).data("type") != "intro") {
            if($(instance.element).data("type") == "last") {
                //console.log(angular.element($("#elasticstack")));
                angular.element($("#elasticstack")).scope().removeIntro();

            }
            else {
                angular.element($("#elasticstack")).scope().vote($(instance.element).data("photo-id"), voteFlag);
            }
            //angular.element($("#elasticstack")).scope().startDrag();
        }
	...
	ElastiStack.prototype._onDragMove = function( instance, event, pointer) {
		...
		if( this._outOfBounds( instance ) ) {
            ...
            //console.log(angular);
            this._moveAway( instance, instance.position.x > 0 );
            instance.element.getElementsByClassName("updown")[0].style.display="none";
            instance.element.getElementsByClassName("updown")[1].style.display="none";
		}
		else {
