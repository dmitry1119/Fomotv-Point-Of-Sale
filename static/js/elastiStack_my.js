...
;( function( window ) {
	...
	ElastiStack.prototype._onDragStart = function( instance, event, pointer ) {
        document.getElementsByClassName("vote-thumbs")[0].style.display="none";
		 var yy=event.target.classList;
        for(var i=0; i<yy.length; i++){
      if(yy[i] == "vote-yes"){
          this._moveAway( instance, true );
          //console.log("vote-up");

          return;
            }
            else  if(yy[i] == "vote-no"){
          this._moveAway( instance, false );
                $(".vote-thumbs").show();
	...
	ElastiStack.prototype._onDragEnd = function( instance, event, pointer ) {
        $(".vote-thumbs").show();
		document.getElementsByClassName("vote-thumbs")[0].style.display="block";
